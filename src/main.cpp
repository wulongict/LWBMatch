...
void printUsage(){
	...
	cout << "options:" << endl;
	cout << "\t-m methodID\t (default 1; 1 for LWBMatch, 2 for DTW )" << endl << "\t-r th_RT\t (default 500; RT tolerance; seconds)" << endl
		<< "\t-z th_MZ\t (default 0.2; MZ tolerance; Th)" << endl << "\t-i th_intensity\t (default 100000000; intensity tolerance)" << endl
		<< "\t-n fileName\t (default 2; the number of sample files)" << endl << "\t-l fileListPath\t (default \"\"; the path of the file containing all the sample files)" << endl
		<< "\t-o outpath\t (default \"\"; the result file)" << endl << "\t-g groundtruth\t (default \"\"; the path of the groundtruth)" << endl
		<< "\t-s separate\t (default 0; separate point for combined datasets. e.g. A1 A2 A3 B1 B2 B3 B4, separatePoint = 3)" << endl
		<< "\t-f faster\t (default 1; set as 1, apply more than 4GB memory for the weight matrix; set as 0, utilizing the sparsity of the weight matrix, ~3 times slower)" << endl
		<< "\t-w warper\t (default 2; 0, no warping; 1, LOWESS warping; 2, MS1 dot product warping; 3, MS1 and MS2 dot product warping)" << endl;
	cout << "Example:" << endl;
	...
	cout << "Example:" << endl;
    cout << "./lwbmatch 1 500 0.2 100000000 20 /path/to/your/FeatureXMLList.txt /path/to/your/resutfile /path/to/the/groundtruth 10" << endl;
	cout << endl;
