...
// This section is not correct in windows system, comment by Landon
// Start
    ...
    for (retryLoop = 2; retryLoop--;) {
        ...
        *iLastScan = 0;
        ...
        if ((indexOffset < 0) || (retryLoop == 0)) { // derive the index by inspection
            ...
            n = 0;
            pScanIndex = (ramp_fileoffset_t *) malloc(
                    sizeof(ramp_fileoffset_t) * reallocSize); // allocate space for the scan index info
            if (!pScanIndex) {
                printf("Cannot allocate memory\n");
                return NULL;
            }
            ...
            while ((nread = (int) ramp_fread(buf, sizeof(buf) - 1, pFI)) > taglen) {
                ...
                while (NULL != (find = strstr(look, scantag))) {
                    ...
                    while (++scanNumStr < buf + sizeof(buf) - 1 &&
                           *scanNumStr != '\"'); // increment until it hits the end quote or the end of buffer
    ...
    if (pFI->bIsMzData) {
        ...
        while (ramp_nextTag(stringBuf, SIZE_BUF, pFI)) {
            ...
            for (attrib = stringBuf - 1; NULL != (attrib = strchr(attrib + 1, '='));) {
                ...
                if (matchAttr(attrib, "cvLabel", 7) || matchAttr(attrib, "accession", 9) ||
                    matchAttr(attrib, "value", 5)) { ; // no info here
        ...
        switch (G_RAMP_OPTION & MASK_SCANS_TYPE) {
            ...
            default:
                ...
                if (scanHeader->mergedScan && 0 != scanHeader->mergedResultScanNum &&
                    scanHeader->acquisitionNum != scanHeader->mergedResultScanNum) {
                    scanHeader->peaksCount = 0;
                }
                break;
        }
...
// Attention, We don not want this function.
char *rampConstructInputPath(char *inbuf, // put the result here
    ...
    char *result = NULL;
    for (int loop = (dir_in && *dir_in) ? 2 : 1;
         loop-- && !result;) { // try it first without directory hint in case basename has full path
        int i;
    ...
    if (pFI->bIsMzData) {
        ...
        while ((!(bGotInten && bGotMZ)) &&
            ...
            if (isArray) {
                ...
                if (peaksCount <= 0) { // No peaks in this scan!!
                    return NULL;
                }
                ...
                if ((!pPeaks) &&
                    ((pPeaks = (RAMPREAL *) malloc((peaksCount + 1) * 2 * sizeof(RAMPREAL) + 1)) == NULL)) {
                    printf("Cannot allocate memory\n");
                    return NULL;
                }
        ...
        while ((!gotMZ) || (!gotIntensity)) {
            ...
            while (1) { // Untill the end of the peaks element
                if ((pBeginData = strstr(buf, "precision="))) { // read the precision attribute
                    precision = atoi(strchr(pBeginData, '\"') + 1);
                }
                if ((pBeginData = strstr(buf, "contentType="))) { // read the contentType attribute
                ...
                if ((pBeginData = strstr(buf, "compressionType="))) { // read the compressionType attribute.
            ...
            if (!precision) { // precision attribute was not defined assume 32 by default
                precision = 32;
            }
            if (mzInt == contType) {
                readingMZ = readingIntensity = gotMZ = gotIntensity = true; // they're munged together
...
//        printf("Decompressing data\n");
            ...
            if (contType == mzRuler) { // Convert back from m/z ruler contentType into m/z - int pairs
                RAMPREAL lastMass = 0;
...
// printf("%f\n" , pPeaks[j] );
                    if ((int) pPeaks[j] == -1) { // Change in delta m/z
                        ++j;
...
InstrumentStruct *getInstrumentStruct(RAMPFILE *pFI) {
    ...
    if (pFI->bIsMzData) {  // TODO
        ...
        while (!strstr(stringBuf, isAncient ? "</instrument" : "</msInstrument") &&
               !strstr(stringBuf, "</dataProcessing") && !ramp_feof(pFI)) {
            if (!found[0]) {
                result = strstr(stringBuf, isAncient ? "manufacturer=" : "<msManufacturer");
                if (result != NULL && setTagValue(result, output->manufacturer, INSTRUMENT_LENGTH,
                                                  isAncient ? "manufacturer=" : "value="))
                    found[0] = 1;
            ...
            if (!found[1]) {
                ...
                if (result != NULL &&
                    setTagValue(result, output->model, INSTRUMENT_LENGTH, isAncient ? "model=" : "value="))
                    found[1] = 1;
            }
            if (!found[2]) {
                result = strstr(stringBuf, isAncient ? "ionisation=" : "<msIonisation");
                if (result != NULL &&
                    setTagValue(result, output->ionisation, INSTRUMENT_LENGTH, isAncient ? "ionisation=" : "value="))
                    found[2] = 1;
            }
            if (!found[3]) {
                result = strstr(stringBuf, isAncient ? "msType=" : "<msMassAnalyzer");
                if (result != NULL &&
                    setTagValue(result, output->analyzer, INSTRUMENT_LENGTH, isAncient ? "msType=" : "value="))
                    found[3] = 1;
            }
            if (!found[4]) {
...
const RAMPREAL *
readPeaksCached(struct ScanCacheStruct *cache, int seqNum, RAMPFILE *pFI, ramp_fileoffset_t lScanIndex) {
    int i = getCacheIndex(cache, seqNum);
    if (cache->peaks[i] == NULL)
