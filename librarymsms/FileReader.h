    ...
    public:
        ...
        virtual bool isValid(PeakFeature *pf, int FileID) {
            ...
            for (int i = 0; i < GetAlignmentNum(); i++) {
                ...
                if (fabs(curpf->GetMZ() - pf->GetMZ()) < m_th_mz && fabs(curpf->GetRT() - pf->GetRT()) < m_th_rt &&
                    fabs(curpf->GetInten() - pf->GetInten()) < m_th_inten) {
                    return true;
    ...
    public:
        pepxmlReader(string input) : Reader(input) {
            if (GetFileName().find("ipro.pep.xml") == string::npos) {
                cout << "ATTENTION: Input file is not ipro.pep.xml format, process of this file may be incorrect!"
                     << endl;
            }
        }
            ...
            while (fin >> line) {
                while (line.find("<error_point") != string::npos) {
                    ...
                    if (fabs(error - 0.01) < 1e-4) {
                        threshold = min_prob;// So, prob is selected by the FDR 1%
                        cout << "threshold=" << threshold << " error=" << error << " num_corr=" << num_corr
                             << " num_incorr=" << num_incorr << endl;
                    }
                }
                ...
                else if (line.find("probability=") != string::npos &&
                         preline.find("<peptideprophet_result") != string::npos)// ToDo
                {
                    line = SplitByFirst(line, '\"')[1];
                    line = SplitByFirst(line, '\"')[0];
                    m_PSMs[i - 1].probability = stod(line);// ToDo, use threshold. i--
                    if (m_PSMs[i - 1].probability < threshold && GetFileName().find("pep.xml") != string::npos &&
                        GetFileName().find("ipro.pep.xml") == string::npos) {
                        // Do nothing here
                        m_PSMs.pop_back();
                        i--;
                    }
                         ...
                         preline.find("<interprophet_result") != string::npos) {
                    //cout << "i-1" << i-1 << endl;
        ...
        void CalcMassDiff() {
            ...
            for (int i = 0; i < m_PSMs.size(); ++i) {
                ...
                massdiff.push_back((m_PSMs[i].precursor_neutral_mass - m_PSMs[i].calc_neutral_pep_mass) /
                                   m_PSMs[i].assumed_charge);
            }
            double avg = calcmean(massdiff);
            double massSTD = calcSTD(massdiff);
            cout << "Mass Deviation:" << endl;
        ...
        void GetGroundTruth(vector<string> filenames) {
            ...
            for (unsigned int i = 0; i < m_PSMs.size(); i++) {
                ...
                for (unsigned int j = 0; j < filenames.size(); j++) {
                    ...
                    if (fabs(mzmean - m_PSMs[i].GetPepMZ()) <
                        3) // GetPrecursorMz(): modyfy here today. there is no difference
                    {
                        rt_mz_list.push_back(
                                m_PSMs[i].GetPepMZ());//.GetPepMZ()); // modify here today: wu long 06/07/2015
                    }
                    else {
